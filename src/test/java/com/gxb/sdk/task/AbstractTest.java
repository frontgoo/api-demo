...
//        调用人行征信授权项
//        return new AuthParm(sequenceNo, AuthItemEnum.CREDIT.getValue(), System.currentTimeMillis(), "周海松", "18258171452", "330682199011221410");

//        调用社保授权项
//        return new AuthParm(sequenceNo, AuthItemEnum.SECURITY.getValue(), System.currentTimeMillis(), "周海松", "18258171452", "330682199011221410");

//        调用学信授权项
//        return new AuthParm(sequenceNo, AuthItemEnum.XUEXIN.getValue(), System.currentTimeMillis(), "周海松", "18258171452", "330682199011221410");
    }
    ...
    private static Map<String, String> getAuthArguments(String token, Map<String, String> params) throws Exception {
        GxbResponse<AuthArgumentsVo> response = !authLoginFinished ? abstractApi.getAuthArguments(params).execute().body() : abstractApi.getAuthAgainArguments(params).execute().body();
        if (response == null || BizRetCode.SUCCESS.getCode() != response.getRetCode()) {
    ...
    private static boolean getImageAuthCaptcha(String token, String argumentName) throws Exception {
        ...
        GxbResponse<CaptchaVo> response = !authLoginFinished ? abstractApi.getAuthCaptcha(params).execute().body() : abstractApi.getAuthAgainCaptcha(params).execute().body();
        if (response == null || BizRetCode.SUCCESS.getCode() != response.getRetCode()) {
    ...
    //接收用户对于下拉选项的输入
    private static void receiveOptionArgumentInput(Map<String, String> inputParams, String title, String hint, List<OptionArgumentVo> optionArgumentList) throws Exception {
        for (OptionArgumentVo optionArgumentVo : optionArgumentList) {
            ...
            inputParams.put(optionArgumentName, inputValue);//注意这里要穿的是 optionArgumentVo对象的argumentName
        }
    }
