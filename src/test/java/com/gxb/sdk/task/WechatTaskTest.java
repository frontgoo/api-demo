...
import com.gxb.sdk.api.WechatApi;
 ...
 */
public class WechatTaskTest extends AbstractGxbTest {
    private static final Logger logger = LoggerFactory.getLogger(WechatTaskTest.class);

    WechatApi wechatApi = gxbApiFactory.newApi(WechatApi.class);
    ...
    protected AuthParm getAuthParm() {
        ...
        return new AuthParm(sequenceNo, "wechat", System.currentTimeMillis(), "周海松", "18258171452", "330682199011221410");
    }


    @Test
    public void doWechatTask() throws Exception {
        // step1:生成token
        final AuthToken authToken = getAuthToken();
        // step2:获取淘宝授权初始化配置，定位地址为杭州
        LoginConfig loginConfig = wechatApi.getInitConf(authToken.getToken(), "120.1621610000", "30.2789730000").execute().body().getData();
    ...
    protected Status refreshLoginQrCode(String token) throws IOException {
        return wechatApi.refreshLoginQrCode(token).execute().body().getData();
    }
    ...
    protected Status getStatus(String token) throws IOException {
        return wechatApi.getStatus(token).execute().body().getData();
    }
