 ...
 */
public class JDTaskTest extends AbstractGxbTest {
    private static final Logger logger = LoggerFactory.getLogger(JDTaskTest.class);
    ...
    protected AuthParm getAuthParm() {
        ...
        return new AuthParm(sequenceNo, "jd", System.currentTimeMillis(), "周海松", "18258171452", "330682********141x");
    }

    @Test
    public void doJDTask() throws Exception {
        // step1:生成token
        final AuthToken authToken = getAuthToken();
        // step2:获取京东授权初始化配置，定位地址为杭州
        LoginConfig loginConfig = ecommerceApi.getJDInitConf(authToken.getToken(), "120.1621610000", "30.2789730000").execute().body().getData();
